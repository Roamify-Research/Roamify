...
\title{Roamify: Changing the Way World Travels}
...
%% article.
\begin{abstract}
  Travel planning is difficult and time-consuming. Current LLM-powered tools like ChatGPT-4 need updated data on the model's training. We developed an AI-driven itinerary plugin, Roamify, to make this process more efficient by collecting blog data and planning an itinerary accounting for user preferences. According to this paper, its methods could convert vacation planning from logistical obstacles to exciting adventures. This paper describes Roamify's methods and promises to revolutionize travel planning. Our findings from a user study indicate three design considerations for future travel assistants: \textbf{D1)} using web scraping to collect news articles regarding a place and including its results in the itinerary, \textbf{D2)} collecting attractions from various blog sites and collecting ratings, and \textbf{D3)} exploiting user preferences in the generation of literary.
